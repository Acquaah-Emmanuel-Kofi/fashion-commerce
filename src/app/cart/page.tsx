...
const CartPage = () => {
  ...
  const [activeView, setActiveView] = useState<"cart" | "favorites">("cart"); // State for active view
  ...
  const handleNext = () => {
    const currentItems = activeView === "cart" ? cartItems : favoriteItems;
        ...
        <div className="mb-8 flex items-center gap-10">
          <h1
            className={`lg:text-lg text-lg font-bold cursor-pointer ${
              activeView === "cart" ? "font-bold" : "font-normal"
            }`}
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`flex justify-between items-center border border-[#D9D9D9] p-2.5 hover:bg-[#D9D9D9]  ${
                activeView === "favorites" ? "bg-black" : "bg-white"
              }`}
            >
              <svg
                width="11"
              ...
              >
                <path
                  ...
                  fill={`${
                    activeView === "favorites" ? "#FFFFFF" : " #1E1E1E"
                  }`}
                />
              </svg>
            </button>
          ...
          {currentItems.length > 0 && (
            ...
            >
              <MdClear size={25} />
              Clear {activeView === "cart" ? "Cart" : "Favorites"}
            </button>
          )}
        </div>
            ...
            {currentItems.length === 0 ? (
              <div className="w-full min-h-[350px] flex justify-center items-center flex-col">
                <p className="text-lg font-semibold flex items-center gap-2">
                  ...
                  Your {activeView === "cart" ? "cart" : "favorites"} is empty!
                </p>
                <p>
                  Explore our products and add them to your{" "}
                  {activeView === "cart" ? "cart" : "favorites"}!
                </p>
              </div>
            ) : (
              <Carousel
