...
import { selectFavoriteItemCount } from "@/redux/features/favoriteSlice";
import { setSelectedItemsToRender } from "@/redux/features/itemsToRenderSlice";

const Navbar = () => {
  const [isOpen, setIsOpen] = useState(false);
  const cartItemCount = useAppSelector(selectCartItemCount);
  const favoriteItemCount = useAppSelector(selectFavoriteItemCount);
  ...
  const handleCartAndFavoritesRoute = (itemsToRender: string) => {
  ...
  return (
    <Fragment>
      <nav className="flex justify-between items-center px-4 py-4 sticky top-0 z-50 bg-white">
        ...
        <div className="flex items-center lg:space-x-4 space-x-2">
          <div className="relative">
            <Link
              href="/cart"
              onClick={() => handleCartAndFavoritesRoute("favorites")}
          ...
          <div className="flex items-center relative">
            <Link
              href={`/cart`}
              onClick={() => handleCartAndFavoritesRoute("cart")}
      ...
      {/* Mobile Sidebar */}
      <div
        className={`fixed inset-0 z-50 bg-gray-900 bg-opacity-50 transition-transform ${
